...
const GlobalReachSection = () => {
  return (
    <section
      id="global_supply"
      className="relative bg-[#4A7A35] font-signika py-14 px-4 sm:px-8 lg:px-16 xl:px-24 overflow-hidden"
    >
      {/* Background texture */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            <motion.div  className="flex items-start" variants={fadeUp}>
              <div className="flex gap-2 shadow-lg bg-white px-4 py-[5px] rounded-2xl">
                <Image src="/two-leafs.svg" alt="leaf" width={18} height={18} />
            ...
            <motion.h2
              variants={fadeUp}
              className="text-4xl sm:text-5xl pb-5 font-extrabold text-white leading-tight"
            >
              Global Reach..
              <br />
              <span className="text-white/90">Reliable Supply</span>
            ...
            <motion.div
              variants={fadeUp}
              className="relative rounded-2xl overflow-hidden w-full aspect-[4/2] sm:aspect-[16/8] lg:aspect-[5/3]"
            >
              <Image
                src="/global-shipping.webp"
                alt="Shipping container being lifted"
                fill
                className="object-cover"
                priority
              />
              ...
              <div className="absolute bottom-0 right-0 w-14 h-14 bg-[#E8D44D] rounded-2xl flex items-center justify-center shadow-lg">
                <Image src="/icons/globe.svg" alt="Global" width={32} height={32} />
              </div>
            </motion.div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <motion.p
              variants={fadeUp}
              className="text-sm sm:text-base text-white/80 leading-relaxed pl-3 mb-4"
            >
               We operate a high-capacity production facility and reliable global
              shipping network, ensuring timely delivery of premium coco
              substrates to international markets. With full export
                ...
                <motion.div key={item.number} variants={fadeUp}>
                  {item.highlight ? (
                    <div className="bg-white rounded-2xl px-5 py-4 flex items-start gap-4">
                      <span className="text-5xl font-extrabold text-[#4A7A35] w-12">
                        {item.number}
                      </span>

                      <div className="flex flex-col sm:flex-row sm:gap-6 flex-1 gap-2">
                        <h3 className="text-base font-bold text-gray-900 min-w-[130px]">
                          {item.title.split("\n").map((line, i) => (
                    ...
                    <div className="flex items-start gap-4 pl-6 py-3 border-b border-white/10 last:border-0">
                      <span className="text-5xl font-extrabold text-white/20 w-12">
                        {item.number}
                      </span>

                      <div className="flex flex-col sm:flex-row sm:gap-6 flex-1 gap-1">
                        <h3 className="text-base font-bold text-white min-w-[130px]">
                          {item.title.split("\n").map((line, i) => (
                            <span key={i} className="block">
                              {line}
                            </span>
